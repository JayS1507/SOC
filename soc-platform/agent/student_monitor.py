    ...
    def _find_browsers(self):
        ...
        # ── macOS: Library paths ──
            ...
            mac_chrome_paths = [
                ...
                f"{home}/Library/Application Support/Google/Chrome/Profile 1/History",
                ...
                f"{home}/Library/Application Support/BraveSoftware/Brave-Browser/Profile 1/History",
            ...
            for p in mac_chrome_paths:
                if os.path.exists(p):
                    bname = "brave" if "brave" in p.lower() else \
                            ...
                            "edge" if "edge" in p.lower() else "chrome"
                    paths.append((bname, p))
                    print(f"[BrowserMonitor] Found {bname} (macOS): {p}")
        ...
        # ── Windows: AppData paths ──
            ...
            if appdata:
                win_chrome_paths = [
                    ...
                    f"{appdata}\\Google\\Chrome\\User Data\\Profile 1\\History",
                    f"{appdata}\\Chromium\\User Data\\Default\\History",
                ...
                for p in win_chrome_paths:
                    if os.path.exists(p):
                        bname = "brave" if "brave" in p.lower() else \
                                ...
                                "edge" if "edge" in p.lower() else "chrome"
                        paths.append((bname, p))
                        print(f"[BrowserMonitor] Found {bname} (Windows): {p}")
...
class ActiveWindowMonitor:
    ...
    def __init__(self):
        ...
        if self._os_type == "Linux":
            ...
            print("[WindowMonitor] macOS detected — using AppleScript ✓")
        elif self._os_type == "Windows":
            print("[WindowMonitor] Windows detected — using GetForegroundWindow")
    ...
    def _get_active_window_mac(self) -> str:
        ...
        try:
            ...
            return "AppleScript_timeout"
        except Exception as e:
            return ""

    def _get_active_window_windows(self) -> str:
        """Windows: Get active window using Windows API."""
        try:
            import ctypes
